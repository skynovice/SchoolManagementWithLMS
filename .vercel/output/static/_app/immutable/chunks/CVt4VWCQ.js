import{w as i}from"./BP_01uF4.js";import{s}from"./DtlnWW8f.js";import{a as f}from"./BlS67Avr.js";const o=i(null),n=i(null),u=i(!0),p=async()=>{try{u.set(!0);const{data:{session:t}}=await s.auth.getSession();t!=null&&t.user&&(o.set(t.user),await c(t.user.id)),s.auth.onAuthStateChange(async(e,r)=>{r!=null&&r.user?(o.set(r.user),await c(r.user.id)):(o.set(null),n.set(null))})}catch(t){console.error("Auth initialization error:",t)}finally{u.set(!1)}},c=async t=>{try{const{data:e,error:r}=await s.from("profiles").select(`
        *,
        zones (
          id,
          name
